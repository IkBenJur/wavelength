...
export default function ScorePicker({ prompt, onLock }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <p className="text-slate-400 text-sm uppercase tracking-widest font-medium">Your turn to score</p>
        <p className="text-slate-300 text-base leading-relaxed">
          Think of something, tell your partner what it is, then pick where it falls on the scale.
        </p>
      </div>
      ...
      >
        {locked ? 'Locked in!' : selected ? `Lock in ${selected}` : 'Select a number'}
      </button>
    </div>
  );
}
